class Solution {
public:
    int rule2(string wordlist, string queries,int max_score, string x){
        ...
        return (score>max_score || x=="")?score:-1;
    }
    vector<string> spellchecker(vector<string>& wordlist, vector<string>& queries) {
        ...
        for(int i=0;i<m;i++){
            ...
            for(int j=0;j<n;j++){
                ...
                else if(rule2(wordlist[j],queries[i],max_score,x)!=-1){
                    max_score+=rule2(wordlist[j],queries[i],max_score,x);
                    x=wordlist[j];
                }
            }
            ans.push_back(x);
